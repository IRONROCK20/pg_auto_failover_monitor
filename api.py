...
@app.route("/monitor")
def monitor():
    settings_output = run_pg_autoctl(["pg_autoctl", "show", "settings", "$PGDATA"])
...
def maintenance_on():
    output = run_pg_autoctl(["pg_autoctl", "enable", "maintenance", "--allow-failover", "$PGDATA"])
    return jsonify({
...
def maintenance_off():
    output = run_pg_autoctl(["pg_autoctl", "disable", "maintenance", "$PGDATA"])
    return jsonify({
